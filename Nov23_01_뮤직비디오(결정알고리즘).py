...
def Count(capacity):
    cnt=1       #DVD의 개수를 1부터 시작
    sum=0
    for x in Music:
        if sum+x>capacity:      #용량을 초과하면
            cnt+=1              #DVD의 개수 1개 증가
            sum=x               #sum은 그 초과한 시점의 x로 초기화
        else:                   #용량과 같거나 작으면
            sum+=x              #sum이 쌓이게
    return cnt      #DVD의 개수 리턴
...
maxx=max(Music)
lt = 1                        #정답 범위 지정
rt = sum(Music)               #rt의 최대값은 sum으로 --> sum의 용량을 가진 m개 가능하니까
res = 0                       #정답을 담는 그릇
while lt<=rt:
    mid=(lt+rt) // 2
    if mid>maxx and Count(mid)<=m:       #Count함수 : 정답을 입력받아서 가능한 DVD의 개수를 출력하는 함수
        res=mid
        rt=mid-1
    else:
        lt=mid+1
